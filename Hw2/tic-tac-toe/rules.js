...
*/
function play() {
	var space = document.getElementById('move_text_id').value;
	if (!started) {
		alert('The game has not started.');
	}
	else {
		if (space!='A1'&&space!='A2'&&space!='A3'&&space!='B1'&&space!='B2'&&space!='B3'&&space!='C1'&&space!='C2'&&space!='C3') {
			alert('Please enter a valid move in the range A1 to C3.');
		}
		else if (document.getElementById(space).innerHTML!=space) {
			...
			else if(((C3==C1)&&(C3==C2))||((C3==A3)&&(C3==B3))){
				alert('Winner is: ' + C3);
				reset_play();
			}
			else if((C1==B2)&&(C1==A3)){
